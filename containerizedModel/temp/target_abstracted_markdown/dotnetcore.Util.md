...
几年前我在博客园写了一个 @abstr_hyperlink 的系列文章，受到不少.net同学的青睐，不过由于种种原因（想写的范围过大，技术更新过快，工作忙，人越来越懒，好吧，我承认主要是最后一点）而烂尾。

我在博客中放出过一些示例代码，由于未正式开源，修复过的bug也未能更新，所以将博客中提供的示例项目都删除了。

一些同学将这些代码应用在项目上，当碰到问题时，由于没有技术支持，也没有相关教程，苦不堪言。而另外一些同学没有下载到示例项目。

虽然博客断更 @abstr_number 年，但时常有同学通过加QQ或发EMAIL的方式联系我，希望能够得到最新代码，并对用法和设计思路进行讲解。

.net core发展迅猛，它为.net提供了更广阔的竞争力，同时也意味着新一轮的学习与代码移植工作开始，我也在这个大潮中跌跌撞撞的摸索着。

几位好友（ @abstr_hyperlink @abstr_hyperlink @abstr_hyperlink ）发起了.net core学习小组，并打算为国内.net core开源大业作出贡献，我果断加入了队伍进行学习，并决定对Util应用框架进行开源。

经讨论，大家决定将各自的开源项目统一发布到 @abstr_hyperlink 之下，以方便国内.net core开源项目的推广，同时也欢迎国内.net core开源项目作者的加入。
...
随着经验和水平的上升，我根据自己的习惯，逐步实现了经典图书中某些概念（比如规约），并吸收了一些流行应用框架( 比如 @abstr_hyperlink @abstr_hyperlink )的部分代码。
...
**强类型（ _静态类型_ ）** 化风格，将UI组件封装为asp.net core razor组件，充分发挥VS开发工具的优势，比如智能提示，编译时检查。
...
Util应用框架面向的是 _架构初学者_ ，但不是.net初学者。
...
@abstr_number . 开发工具： \- Visual Studio @abstr_number \- @abstr_hyperlink 

@abstr_number . 数据库 \- Sql Server \- Mysql \- PostgreSQL

@abstr_number . 设计工具 \- PowerDesigner @abstr_number . @abstr_number \- XMind

@abstr_number . 版本控制 \- Git

@abstr_number . 部署环境 \- Ubuntu Server \- Docker

@abstr_number . SDK及Runtime \- 当前SDK @abstr_number 位版本： @abstr_hyperlink ，开发机器安装 \- 当前Runtime @abstr_number 位版本： @abstr_hyperlink ，服务器安装

@abstr_number . 单元测试及模拟框架 \- XUnit \- NSubstitute

@abstr_number . 数据访问框架 \- @abstr_hyperlink \- @abstr_hyperlink 

@abstr_number . Ioc框架 \- Autofac

@abstr_number . Aop框架 \- @abstr_hyperlink 

@abstr_number . Json框架 \- Json.Net

@abstr_number . 映射框架 \- AutoMapper

@abstr_number . 日志框架 \- @abstr_hyperlink \- @abstr_hyperlink 

@abstr_number . 缓存 \- @abstr_hyperlink 

@abstr_number . 事件总线 \- @abstr_hyperlink 

@abstr_number . Id生成器 \- @abstr_hyperlink 

@abstr_number . RSA加密算法 \- @abstr_hyperlink 

@abstr_number . 二维码操作 \- @abstr_hyperlink 

@abstr_number . 短信操作 \- @abstr_hyperlink 

@abstr_number . Queryable动态扩展 \- @abstr_hyperlink 

@abstr_number . Web框架 \- @abstr_hyperlink 

@abstr_number . 定时任务调度框架 \- @abstr_hyperlink 

@abstr_number . UI \- 脚本语言 \- @abstr_hyperlink \- 脚本框架 \- @abstr_hyperlink \- Css预处理器 \- @abstr_hyperlink \- @abstr_hyperlink \- 组件库 \- @abstr_hyperlink \- @abstr_hyperlink \- @abstr_hyperlink \- @abstr_hyperlink \- 编辑器 \- @abstr_hyperlink \- @abstr_hyperlink   
\- 脚本库 \- @abstr_hyperlink \- @abstr_hyperlink \- 图标 \- @abstr_hyperlink \- @abstr_hyperlink \- @abstr_hyperlink   
\- 图表 \- @abstr_hyperlink \- @abstr_hyperlink \- 打包 \- Webpack \- 测试 \- Karma \- Jasmine

@abstr_number . 权限 \- @abstr_hyperlink \- @abstr_hyperlink 

@abstr_number . 代码生成器 \- CodeSmith @abstr_number . @abstr_number 

@abstr_number . 参考应用框架 \- @abstr_hyperlink \- @abstr_hyperlink \- Nop是一个开源商城，封装了大量实用的基础代码。
...
对于权限，目前发现最通用，最简单，最易理解的方法是基于 _资源_ 和 _角色_ 的权限设计。操作权限是通用的，但数据权限太灵活，目前我们配合规约模式来解决。
...
当你在使用Util进行开发时，倘若碰到无法解决的问题，可以加群讨论，Util团队会尽力帮助你度过难关。
  ...
  * Util应用框架交流QQ群(一群)： @abstr_number 

> 不论是Util应用框架的代码，还是交流活动都是免费的，所以你不能指望我们必须在短时间内响应你，同时也希望大家都保持开放的心态，为初学者提供一些便利。
> 
> 对于一些伸手党，本来百度 @abstr_number 分钟就可以解决的问题，却四处找人问，对于这些家伙，我建议你们转JAVA。
