...
## Getting Started with Proton from Steam Play
  ...
  * As the new Steam Play is still in Beta, it is recommended that you opt into the @abstr_hyperlink for the latest features and fixes.
  * Proton requires graphics drivers that are more recent than what is typically packaged in most distributions; please read the @abstr_hyperlink 
  * Install and play games! Please refer to @abstr_hyperlink for more information.
  * Visit the @abstr_hyperlink for information about recent updates.
...
The most current source for Proton is here: @abstr_hyperlink 
...
`make module=<module> module` \- This will build both @abstr_number - and @abstr_number -bit versions of the specified module, and copy the result into the `vagrant_share` directory. This allows rapid iteration on one module. This target is only useful after building Proton.
...
@abstr_number . Set up your build environment @abstr_number . Configure the build @abstr_number . Build Proton @abstr_number . Install Proton locally (optional)
...
Proton has a lot of build-time dependencies. The following instructions describe how we create the build environment for the production builds of Proton. For reproducibility and security reasons, we will be setting up a Debian @abstr_number virtual machine. However, you should be able to follow these instructions on other distributions as well.

Proton provides a Vagrantfile, which will automatically set up the Debian @abstr_number VM for you. After installing @abstr_hyperlink , initialize the VM by running from within the Proton directory:
...
If you do not wish to use Vagrant, you can read through both Vagrantfile and `vagrant-user-setup.sh` for the list of dependencies and instructions on how to set up your own machine or another VM of your choosing. It is aimed at Debian @abstr_number , but you should be able to adapt them for other distributions.
...
After setting up the build system, it is time to run the configure script which will generate the Makefile to build your project. Run these steps. You may of course use whatever paths you like.
        ...
        mkdir build/
        cd build
        ../proton/configure.sh --steam-runtime @abstr_number =docker:steam-proton-dev --steam-runtime @abstr_number =docker:steam-proton-dev @abstr_number  --steam-runtime=$HOME/steam-runtime/runtime/
...
**Tip** : If you are building without the Steam runtime, you should now run `make obj-wine @abstr_number /Makefile obj-wine @abstr_number /Makefile` and check the files `obj-wine @abstr_number /config.log` and `obj-wine @abstr_number /config.log` for missing packages. Search for `won't be supported`. A couple of missing packages are normal: `opencv`, `gstreamer`, `vkd @abstr_number d`, `oss`. More than that may indicate a problem. Please see your distro's documentation to acquire the considerable build dependencies for Wine.
...
| Compat config string | Environment Variable | Description | | :-------------------- | :----------------------------- | :----------- | | | `PROTON_LOG` | Convenience method for dumping a useful debug log to `$HOME/steam-$APPID.log`. For more thorough logging, use `user_settings.py`. | | | `PROTON_DUMP_DEBUG_COMMANDS` | When running a game, Proton will write some useful debug scripts for that game into `$PROTON_DEBUG_DIR/proton_$USER/`. | | | `PROTON_DEBUG_DIR` | Root directory for the Proton debug scripts, `/tmp` by default. | | `wined @abstr_number d` | `PROTON_USE_WINED @abstr_number D` | Use OpenGL-based wined @abstr_number d instead of Vulkan-based DXVK for d @abstr_number d @abstr_number and d @abstr_number d @abstr_number . This used to be called `PROTON_USE_WINED @abstr_number D @abstr_number`, which is now an alias for this same option. | | `nod @abstr_number d @abstr_number ` | `PROTON_NO_D @abstr_number D @abstr_number ` | Disable `d @abstr_number d @abstr_number .dll`, for d @abstr_number d @abstr_number games which can fall back to and run better with d @abstr_number d @abstr_number . | | `nod @abstr_number d @abstr_number ` | `PROTON_NO_D @abstr_number D @abstr_number ` | Disable `d @abstr_number d @abstr_number .dll` and `dxgi.dll`, for d @abstr_number d @abstr_number games which can fall back to and run better with d @abstr_number d @abstr_number . | | `noesync` | `PROTON_NO_ESYNC` | Do not use eventfd-based in-process synchronization primitives. | | `forcelgadd` | `PROTON_FORCE_LARGE_ADDRESS_AWARE` | Force Wine to enable the LARGE_ADDRESS_AWARE flag for all executables. | | `oldglstr` | `PROTON_OLD_GL_STRING` | Set some driver overrides to limit the length of the GL extension string, for old games that crash on very long extension strings. |
