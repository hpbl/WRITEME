...
`SqQueue.rear = (SqQueue.rear + @abstr_number ) % SqQueue.maxSize`
...
@abstr_number . 为了保证客户端发送的最后一个 ACK 报文能够到达服务端。若未成功到达，则服务端超时重传 FIN+ACK 报文段，客户端再重传 ACK，并重新计时。 @abstr_number . 防止已失效的连接请求报文段出现在本连接中。TIME-WAIT 持续 @abstr_number MSL 可使本连接持续的时间内所产生的所有报文段都从网络中消失，这样可使下次连接中不会出现旧的连接报文段。
...
方法 | 意义 \--- | --- OPTIONS | 请求一些选项信息，允许客户端查看服务器的性能 GET | 请求指定的页面信息，并返回实体主体 HEAD | 类似于 get 请求，只不过返回的响应中没有具体的内容，用于获取报头 POST | 向指定资源提交数据进行处理请求（例如提交表单或者上传文件）。数据被包含在请求体中。POST请求可能会导致新的资源的建立和/或已有资源的修改 PUT | 从客户端向服务器传送的数据取代指定的文档的内容 DELETE | 请求服务器删除指定的页面 TRACE | 回显服务器收到的请求，主要用于测试或诊断

状态码（Status-Code）

  * @abstr_number xx：表示通知信息，如请求收到了或正在进行处理 
    * @abstr_number Continue：继续，客户端应继续其请求
    * @abstr_number Switching Protocols 切换协议。服务器根据客户端的请求切换协议。只能切换到更高级的协议，例如，切换到 HTTP 的新版本协议
  * @abstr_number xx：表示成功，如接收或知道了 
    * @abstr_number OK: 请求成功
  * @abstr_number xx：表示重定向，如要完成请求还必须采取进一步的行动 
    * @abstr_number Moved Permanently: 永久移动。请求的资源已被永久的移动到新 URL，返回信息会包括新的 URL，浏览器会自动定向到新 URL。今后任何新的请求都应使用新的 URL 代替
  * @abstr_number xx：表示客户的差错，如请求中有错误的语法或不能完成 
    * @abstr_number Bad Request: 客户端请求的语法错误，服务器无法理解
    * @abstr_number Unauthorized: 请求要求用户的身份认证
    * @abstr_number Forbidden: 服务器理解请求客户端的请求，但是拒绝执行此请求（权限不够）
    * @abstr_number Not Found: 服务器无法根据客户端的请求找到资源（网页）。通过此代码，网站设计人员可设置 “您所请求的资源无法找到” 的个性页面
    * @abstr_number Request Timeout: 服务器等待客户端发送的请求时间过长，超时
  * @abstr_number xx：表示服务器的差错，如服务器失效无法完成请求 
    * @abstr_number Internal Server Error: 服务器内部错误，无法完成请求
    * @abstr_number Service Unavailable: 由于超载或系统维护，服务器暂时的无法处理客户端的请求。延时的长度可包含在服务器的 Retry-After 头信息中
    * @abstr_number Gateway Timeout: 充当网关或代理的服务器，未及时从远端服务器获取请求



> 更多状态码： @abstr_hyperlink 

##### 其他协议

  * SMTP（Simple Main Transfer Protocol，简单邮件传输协议）是在 Internet 传输 Email 的标准，是一个相对简单的基于文本的协议。在其之上指定了一条消息的一个或多个接收者（在大多数情况下被确认是存在的），然后消息文本会被传输。可以很简单地通过 Telnet 程序来测试一个 SMTP 服务器。SMTP 使用 TCP 端口 @abstr_number 。
...
##### read()
  ...
  * 如果返回的值是 @abstr_number 表示已经读到文件的结束了，小于 @abstr_number 表示出现了错误。
...
##### write()
  ...
  * 成功时返回写的字节数。失败时返回 - @abstr_number ，并设置 errno 变量。
  * 在网络程序中，当我们向套接字文件描述符写时有俩种可能。
  * （ @abstr_number ）write 的返回值大于 @abstr_number ，表示写了部分或者是全部的数据。
  * （ @abstr_number ）返回的值小于 @abstr_number ，此时出现了错误。
...
@abstr_number . 客户端向服务器发送一个 SYN J @abstr_number . 服务器向客户端响应一个 SYN K，并对 SYN J 进行确认 ACK J+ @abstr_number @abstr_number . 客户端再想服务器发一个确认 ACK K+ @abstr_number 

只有就完了三次握手，但是这个三次握手发生在 Socket 的那几个函数中呢？请看下图：

@abstr_image 

从图中可以看出： @abstr_number . 当客户端调用 connect 时，触发了连接请求，向服务器发送了 SYN J 包，这时 connect 进入阻塞状态；   
@abstr_number . 服务器监听到连接请求，即收到 SYN J 包，调用 accept 函数接收请求向客户端发送 SYN K ，ACK J+ @abstr_number ，这时 accept 进入阻塞状态；   
@abstr_number . 客户端收到服务器的 SYN K ，ACK J+ @abstr_number 之后，这时 connect 返回，并对 SYN K 进行确认；   
@abstr_number . 服务器收到 ACK K+ @abstr_number 时，accept 返回，至此三次握手完毕，连接建立。
...
@abstr_number . 某个应用进程首先调用 close 主动关闭连接，这时 TCP 发送一个 FIN M； @abstr_number . 另一端接收到 FIN M 之后，执行被动关闭，对这个 FIN 进行确认。它的接收也作为文件结束符传递给应用进程，因为 FIN 的接收意味着应用进程在相应的连接上再也接收不到额外数据； @abstr_number . 一段时间之后，接收到文件结束符的应用进程调用 close 关闭它的 socket。这导致它的 TCP 也发送一个 FIN N； @abstr_number . 接收到这个 FIN 的源发送端 TCP 对它进行确认。

这样每个方向上都有一个 FIN 和 ACK。

## 💾 数据库

> 本节部分知识点来自《数据库系统概论（第 @abstr_number 版）》
...
### 常用数据模型
  ...
  * 关系模型（relational model） 
    ...
    * 分量：元组中的一个属性值
    * 关系模式：对关系的描述，一般表示为 `关系名(属性 @abstr_number , 属性 @abstr_number , ..., 属性n)`
...
#### 范式
  ...
  * 第一范式（ @abstr_number NF）：属性（字段）是最小单位不可再分。
  * 第二范式（ @abstr_number NF）：满足 @abstr_number NF，每个非主属性完全依赖于主键（消除 @abstr_number NF 非主属性对码的部分函数依赖）。
  * 第三范式（ @abstr_number NF）：满足 @abstr_number NF，任何非主属性不依赖于其他非主属性（消除 @abstr_number NF 主属性对码的传递函数依赖）。
  * 鲍依斯-科得范式（BCNF）：满足 @abstr_number NF，任何非主属性不能对主键子集依赖（消除 @abstr_number NF 主属性对码的部分和传递函数依赖）。
  * 第四范式（ @abstr_number NF）：满足 @abstr_number NF，属性之间不能有非平凡且非函数依赖的多值依赖（消除 @abstr_number NF 非平凡且非函数依赖的多值依赖）。
...
> 各大设计模式例子参考： @abstr_hyperlink 系列博文
...
一般应用程序内存空间有如下区域：
  ...
  * 保留区：保留区并不是一个单一的内存区域，而是对内存中受到保护而禁止访问的内存区域的总称，如通常 C 语言讲无效指针赋值为 @abstr_number （NULL），因此 @abstr_number 地址正常情况下不可能有效的访问数据
...
平台 | 可执行文件 | 目标文件 | 动态库/共享对象 | 静态库 \---|---|---|---|--- Windows|exe|obj|dll|lib Unix/Linux|ELF、out|o|so|a Mac|Mach-O|o|dylib、tbd、framework|a、framework

#### 编译链接过程

@abstr_number . 预编译（预编译器处理如 `#include`、`#define` 等预编译指令，生成 `.i` 或 `.ii` 文件） @abstr_number . 编译（编译器进行词法分析、语法分析、语义分析、中间代码生成、目标代码生成、优化，生成 `.s` 文件） @abstr_number . 汇编（汇编器把汇编码翻译成机器码，生成 `.o` 文件） @abstr_number . 链接（连接器进行地址和空间分配、符号决议、重定位，生成 `.out` 文件）

> 现在版本 GCC 把预编译和编译合成一步，预编译编译程序 cc @abstr_number 、汇编器 as、连接器 ld
> 
...
段 | 功能 \--- | --- File Header | 文件头，描述整个文件的文件属性（包括文件是否可执行、是静态链接或动态连接及入口地址、目标硬件、目标操作系统等） .text section | 代码段，执行语句编译成的机器代码 .data section | 数据段，已初始化的全局变量和局部静态变量 .bss section | BSS 段（Block Started by Symbol），未初始化的全局变量和局部静态变量（因为默认值为 @abstr_number ，所以只是在此预留位置，不占空间） .rodata section | 只读数据段，存放只读数据，一般是程序里面的只读变量（如 const 修饰的变量）和字符串常量 .comment section | 注释信息段，存放编译器版本信息 .note.GNU-stack section | 堆栈提示段 
...
Symbol（符号名） | Symbol Value （地址） \--- | --- main| @abstr_number x @abstr_number Add | @abstr_number x @abstr_number ... | ...
...
应用程序类型|入口点函数|嵌入可执行文件的启动函数 \---|---|--- 处理ANSI字符（串）的GUI应用程序|_tWinMain(WinMain)|WinMainCRTSartup 处理Unicode字符（串）的GUI应用程序|_tWinMain(wWinMain)|wWinMainCRTSartup 处理ANSI字符（串）的CUI应用程序|_tmain(Main)|mainCRTSartup 处理Unicode字符（串）的CUI应用程序|_tmain(wMain)|wmainCRTSartup 动态链接库（Dynamic-Link Library）|DllMain|_DllMainCRTStartup 
...
@abstr_number . 包含可执行文件的目录 @abstr_number . Windows 的系统目录，可以通过 GetSystemDirectory 得到 @abstr_number . @abstr_number 位的系统目录，即 Windows 目录中的 System 子目录 @abstr_number . Windows 目录，可以通过 GetWindowsDirectory 得到 @abstr_number . 进程的当前目录 @abstr_number . PATH 环境变量中所列出的目录
...
在 `VS 的开发人员命令提示符` 使用 `DumpBin.exe` 可查看 DLL 库的导出段（导出的变量、函数、类名的符号）、相对虚拟地址（RVA，relative virtual address）。如： @abstr_code_section 
...
@abstr_number . 操作系统创建进程，把控制权交给程序的入口（往往是运行库中的某个入口函数） @abstr_number . 入口函数对运行库和程序运行环境进行初始化（包括堆、I/O、线程、全局变量构造等等）。 @abstr_number . 入口函数初始化后，调用 main 函数，正式开始执行程序主体部分。 @abstr_number . main 函数执行完毕后，返回到入口函数进行清理工作（包括全局变量析构、堆销毁、关闭I/O等），然后进行系统调用结束进程。
...
@abstr_number . 初始化和 OS 版本有关的全局变量。 @abstr_number . 初始化堆。 @abstr_number . 初始化 I/O。 @abstr_number . 获取命令行参数和环境变量。 @abstr_number . 初始化 C 库的一些数据。 @abstr_number . 调用 main 并记录返回值。 @abstr_number . 检查错误并将 main 的返回值返回。
...
> @abstr_hyperlink ：📚 Computer Science Books 计算机技术类书籍 PDF
...
@abstr_number . 硕士及以上学历，计算机、信号处理、数学、信息类及相关专业和方向； @abstr_number . 视频编解码基础扎实，熟常用的 HEVC 或 H @abstr_number ，有较好的数字信号处理基础； @abstr_number . 掌握 C/C++，代码能力强, 熟悉一种汇编语言尤佳； @abstr_number . 较强的英文文献阅读能力； @abstr_number . 学习能力强，具有团队协作精神，有较强的抗压能力。
...
## 👍 内推
  ...
  * @abstr_hyperlink ：🔎 互联网内推信息（社招、校招、实习）
  * @abstr_hyperlink ：国内公司人工智能方向（含机器学习、深度学习、计算机视觉和自然语言处理）岗位的招聘信息（含全职、实习和校招）
...
@abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 、 @abstr_hyperlink 
...
本仓库遵循 CC BY-NC-SA @abstr_number . @abstr_number （署名 - 非商业性使用） 协议，转载请注明出处。
