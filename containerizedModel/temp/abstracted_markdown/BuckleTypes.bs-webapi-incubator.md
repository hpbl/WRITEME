...
@abstr_code_section Then add `bs-webapi` to `bs-dependencies` in your `bsconfig.json`. A minimal example: @abstr_code_section 
...
The Dom types, and the relationships between them, are actually defined in the `Dom` module that ships with `bs-platform` ( @abstr_hyperlink ), where you'll find a bunch of types that look like this:

@abstr_code_section 

This is subtyping implemented with abstract types and phantom arguments. The details of how this works isn't very important (but see @abstr_hyperlink for a detailed explanation of how exactly this trickery works) in order to just use them, but there are a few things you should know:
